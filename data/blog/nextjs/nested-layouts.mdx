...
Before diving into nested layouts, let's understand how to create a basic layout in Next.js. A layout in Next.js is typically a React component that wraps around your page content. Here’s a simple layout component that in our Bar page:
...
Then, when defining the pages that should use both the `GeneralLayout` and the `BarLayout`, you can nest them like this:
...
const SomePage = () => {
  return (
    <GeneralLayout>
      <BarLayout>
        <p>This is the content specific to Some Page which uses BarLayout.</p>
      </BarLayout>
    </GeneralLayout>
  );
};
